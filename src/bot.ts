...
(async () => {
  ...
  client.once('ready', async () => {
    console.log(i18next.t('console.ready', { tag: client.user?.tag }));
  ...
  client.on('interactionCreate', async (interaction: Interaction) => {
    ...
    try {
      ...
      console.error(i18next.t('console.errorCreatingEvent'), err);
      const errorMessage = i18next.t('error.general');
  ...
  try {
    console.log(i18next.t('console.registeringCommands'));
    await rest.put(Routes.applicationGuildCommands(CLIENT_ID, GUILD_ID), {
      body: [standbyCommand.toJSON()],
    });
    console.log(i18next.t('console.commandsRegistered'));
  } catch (error) {
    console.error(i18next.t('console.commandRegistrationError'), error);
  }
